...
const RESOURCE_NAME: &str = "site_replicates";
...
#[utoipa::path(
    post,
    path = format!("/api/{}", RESOURCE_NAME),
    responses((status = CREATED, body = super::models::SiteReplicate))
)]
pub async fn create_one(
    State(db): State<DatabaseConnection>,
    Json(payload): Json<super::models::SiteReplicateCreate>,
) -> Result<(StatusCode, Json<super::models::SiteReplicate>), (StatusCode, Json<String>)> {
    let new_obj: super::db::ActiveModel = payload.into();

    match super::db::Entity::insert(new_obj).exec(&db).await {
        Ok(insert_result) => {
            let response_obj: super::models::SiteReplicate =
...
pub async fn update_one(
    State(db): State<DatabaseConnection>,
    Path(id): Path<Uuid>,
    Json(payload): Json<super::models::SiteReplicateUpdate>,
) -> impl IntoResponse {
    ...
    let response_obj: super::models::SiteReplicate = updated_obj.update(&db).await.unwrap().into();
