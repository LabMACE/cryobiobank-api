...
#[utoipa::path(
    post,
    path = format!("/api/{}", RESOURCE_NAME),
    responses((status = CREATED, body = super::models::DNA))
)]
pub async fn create_one(
    State(db): State<DatabaseConnection>,
    Json(payload): Json<super::models::DNACreate>,
) -> Result<(StatusCode, Json<super::models::DNA>), (StatusCode, Json<String>)> {
    let new_obj: super::db::ActiveModel = payload.into();

    match super::db::Entity::insert(new_obj).exec(&db).await {
        Ok(insert_result) => {
            let response_obj: super::models::DNA =
...
pub async fn update_one(
    State(db): State<DatabaseConnection>,
    Path(id): Path<Uuid>,
    Json(payload): Json<super::models::DNAUpdate>,
) -> impl IntoResponse {
    ...
    let response_obj: super::models::DNA = updated_obj.update(&db).await.unwrap().into();
